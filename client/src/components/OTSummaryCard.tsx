...
const OTSummaryCard: React.FC<OTSummaryCardProps> = ({ ot, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex justify-center items-center p-4">
      <div className="relative bg-white w-full max-w-2xl md:max-w-3xl lg:max-w-4xl mx-auto rounded-lg shadow-xl p-6">
        {/* Card Header */}
        ...
        {/* Card Content */}
        <div className="space-y-4 max-h-[calc(100vh-12rem)] overflow-y-auto pr-2"> {/* Scrollable content area */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 text-sm">
            <div><strong>Cliente:</strong> {ot.cliente || 'N/A'}</div>
            <div><strong>Nº Serie:</strong> {ot.numeroSerie || 'N/A'}</div>
            <div><strong>Modelo:</strong> {ot.modelo || 'N/A'}</div>
            <div><strong>Estado Actual:</strong> <span className="font-semibold">{ot.estado || 'N/A'}</span></div>
          </div>
